...
#### Note that the server automatically generates a public key and stores it in ~/.config/retrospectrum. This public key must be shared with clients so they can verify the server's identity when connecting.
