...
static void admin_text(SDL_Renderer *renderer, TTF_Font *font, const char *text, int x, int y,
                       SDL_Color color) {
    SDL_Surface *surface;
    SDL_Texture *texture;
...
static void admin_centered(SDL_Renderer *renderer, TTF_Font *font, const char *text, SDL_Rect rect,
                           SDL_Color color) {
    int width = 0;
    int height = 0;
    ...
    admin_text(renderer, font, text, rect.x + (rect.w - width) / 2,
               rect.y + (rect.h - height) / 2, color);
}

static void admin_button(SDL_Renderer *renderer, TTF_Font *font, SDL_Rect rect, const char *label,
                         int hovered, SDL_Color color) {
...
static void admin_field(SDL_Renderer *renderer, TTF_Font *font, SDL_Rect rect, const char *label,
    ...
    if (active && ((SDL_GetTicks64() / 500U) % 2U) == 0U) {
        ...
        SDL_RenderDrawLine(renderer, rect.x + 13 + width, rect.y + 8,
                           rect.x + 13 + width, rect.y + rect.h - 8);
    }
}
...
static int admin_refresh_users(Type_Auth_Admin_State *state) {
    ...
    if (!state->bootstrap_mode && !admin_role_is_privileged(state->acting_role)) {
        admin_set_status(state, Admin_ERROR,
                         "This account no longer has administrator privileges.");
        return 0;
    }
...
static int admin_selected_valid(const Type_Auth_Admin_State *state) {
    return state && state->selected >= 0 &&
           (size_t)state->selected < state->user_count;
}
...
static int admin_can_delete_selected(const Type_Auth_Admin_State *state) {
    ...
    target = &state->users[state->selected];
    return target->role != AUTH_ROLE_ADMIN &&
           strcmp(target->username, state->acting_admin) != 0;
}
...
static SDL_Rect admin_main_panel(int width, int height) {
    ...
    return (SDL_Rect){(width - panel_width) / 2, (height - panel_height) / 2,
...
static void admin_render_users(SDL_Renderer *renderer, TTF_Font *font_small, TTF_Font *font_medium,
    ...
    SDL_Rect remove_totp_button = {set_totp_button.x + set_totp_button.w + 12, set_totp_button.y,
                                   set_totp_button.w, 40};
    ...
    for (int row = 0; row < visible; row++) {
        ...
        admin_outline(renderer, row_rect,
                      admin_point_in_rect(mouse_x, mouse_y, row_rect) || index == state->selected
                          ? Admin_ACTIVE
                          : Admin_BORDER);
        admin_text(renderer, font_small, state->users[index].username, row_rect.x + 12, row_rect.y + 11,
                   Admin_TEXT);
        if (state->users[index].role == AUTH_ROLE_ADMIN) {
            admin_text(renderer, font_small, "ADMIN", row_rect.x + row_rect.w - 70,
                       row_rect.y + 11, Admin_WARN);
        } else if (state->users[index].role == AUTH_ROLE_CO_ADMIN) {
            admin_text(renderer, font_small, "CO-ADMIN", row_rect.x + row_rect.w - 98,
                       row_rect.y + 11, Admin_COADMIN);
        }
    }
    ...
    if (selected_valid) {
        ...
        if (localtime_r(&created_time, &local_time) &&
            strftime(created, sizeof(created), "%Y-%m-%d %I:%M:%S %p",
                     &local_time) > 0) {
            char *time_part = strchr(created, ' ');
            if (time_part && time_part[1] == '0') {
                memmove(time_part + 1, time_part + 2,
                        strlen(time_part + 2) + 1);
            }
        }
        ...
        admin_text(renderer, font_small, line, details.x + 30, details.y + 56,
                   admin_role_color(user->role));
    ...
    admin_button(renderer, font_small, role_button, role_button_label,
                 can_change_role && admin_point_in_rect(mouse_x, mouse_y, role_button),
                 role_button_color);
    ...
    if (state->status[0]) {
        admin_centered(renderer, font_small, state->status,
                       (SDL_Rect){panel.x + 24, panel.y + panel.h - 42, panel.w - 48, 24},
                       state->status_color);
    }
}
...
static void admin_render_create(SDL_Renderer *renderer, TTF_Font *font_small, TTF_Font *font_medium,
    ...
    admin_centered(renderer, font_medium,
                   state->bootstrap_mode ? "INITIALIZE ADMINISTRATOR" : "CREATE USER ACCOUNT",
    ...
    admin_text(renderer, font_small, "Require authenticator-app 2FA", checkbox.x + 34, checkbox.y + 2,
               Admin_TEXT);
    ...
    if (state->status[0]) {
        admin_centered(renderer, font_small, state->status,
                       (SDL_Rect){panel.x + 30, panel.y + panel.h - 115, panel.w - 60, 24},
                       state->status_color);
    }
}
...
static void admin_render_totp(SDL_Renderer *renderer, TTF_Font *font_small, TTF_Font *font_medium,
    ...
    admin_centered(renderer, font_medium,
                   state->view == AUTH_ADMIN_VIEW_SET_TOTP
                       ? "SET / REPLACE TWO-FACTOR AUTHENTICATION"
    ...
    admin_centered(renderer, font_small, grouped,
                   (SDL_Rect){secret_box.x + 8, secret_box.y, secret_box.w - 16, secret_box.h},
                   Admin_TEXT);
    ...
    admin_button(renderer, font_small, copy, "COPY SECRET", admin_point_in_rect(mouse_x, mouse_y, copy),
                 Admin_TEXT);
    admin_field(renderer, font_small, code, "Six-digit authenticator code", state->code,
                state->active_field == AUTH_ADMIN_FIELD_CODE, 0);
    admin_button(renderer, font_small, cancel, "CANCEL", admin_point_in_rect(mouse_x, mouse_y, cancel),
                 Admin_MUTED);
...
static void admin_render_reset(SDL_Renderer *renderer, TTF_Font *font_small, TTF_Font *font_medium,
    ...
    admin_centered(renderer, font_small,
                   "Resetting a password disables that account's existing 2FA enrollment.",
    ...
    admin_button(renderer, font_small, cancel, "CANCEL", admin_point_in_rect(mouse_x, mouse_y, cancel),
                 Admin_MUTED);
    admin_button(renderer, font_small, save, "RESET PASSWORD", admin_point_in_rect(mouse_x, mouse_y, save),
                 Admin_WARN);
...
static void admin_render_remove_totp(SDL_Renderer *renderer, TTF_Font *font_small,
                                     TTF_Font *font_medium, Type_Auth_Admin_State *state,
    ...
    admin_centered(renderer, font_small, line,
                   (SDL_Rect){panel.x + 30, panel.y + 170, panel.w - 60, 28}, Admin_TEXT);
    admin_centered(renderer, font_small,
                   "The account will require only its password until 2FA is enrolled again.",
                   (SDL_Rect){panel.x + 30, panel.y + 218, panel.w - 60, 28}, Admin_WARN);
    admin_button(renderer, font_small, cancel, "CANCEL",
                 admin_point_in_rect(mouse_x, mouse_y, cancel), Admin_MUTED);
    admin_button(renderer, font_small, confirm, "REMOVE 2FA",
                 admin_point_in_rect(mouse_x, mouse_y, confirm), Admin_ERROR);
...
static void admin_render_delete(SDL_Renderer *renderer, TTF_Font *font_small, TTF_Font *font_medium,
    ...
    admin_fill(renderer, panel, Admin_PANEL);
    admin_outline(renderer, panel, Admin_ERROR);
    admin_centered(renderer, font_medium, "DELETE USER ACCOUNT",
                   (SDL_Rect){panel.x, panel.y + 40, panel.w, 28}, Admin_ERROR);
    snprintf(line, sizeof(line), "Permanently delete account: %s", state->username);
    admin_centered(renderer, font_small, line, (SDL_Rect){panel.x + 30, panel.y + 170, panel.w - 60, 28},
                   Admin_TEXT);
    admin_centered(renderer, font_small, "This action cannot be undone.",
                   (SDL_Rect){panel.x + 30, panel.y + 218, panel.w - 60, 28}, Admin_WARN);
    admin_button(renderer, font_small, cancel, "CANCEL", admin_point_in_rect(mouse_x, mouse_y, cancel),
                 Admin_MUTED);
    admin_button(renderer, font_small, confirm, "DELETE", admin_point_in_rect(mouse_x, mouse_y, confirm),
                 Admin_ERROR);
...
static void admin_render(SDL_Window *window, SDL_Renderer *renderer, TTF_Font *font_small,
                         TTF_Font *font_medium, Type_Auth_Admin_State *state) {
    int width = 0;
    int height = 0;
    int mouse_x = 0;
...
static void admin_cycle(Type_Auth_Admin_State *state) {
    ...
    if (state->view == AUTH_ADMIN_VIEW_CREATE) {
        ...
        state->active_field = state->active_field == AUTH_ADMIN_FIELD_PASSWORD
                                  ? AUTH_ADMIN_FIELD_CONFIRM
                                  : AUTH_ADMIN_FIELD_PASSWORD;
    } else if (state->view == AUTH_ADMIN_VIEW_CREATE_TOTP ||
               state->view == AUTH_ADMIN_VIEW_SET_TOTP) {
...
static int admin_submit_create(Type_Auth_Admin_State *state) {
    ...
    if (!admin_username_valid(state->username)) {
        admin_set_status(state, Admin_ERROR,
                         "Username: 3-63 letters, numbers, periods, hyphens, or underscores.");
        return 0;
    }
    ...
    if (!AUTH_DB_create_user(state->username, state->password, state->enable_two_factor, is_admin,
                             ...
                             state->bootstrap_mode ? NULL : state->acting_admin,
                             error, sizeof(error))) {
...
static void admin_submit_set_totp(Type_Auth_Admin_State *state) {
    ...
    if (!state || !state->totp_secret_valid ||
        !AUTH_TOTP_verify(state->totp_secret, state->code)) {
        if (state) {
            admin_secure_zero(state->code, sizeof(state->code));
            admin_set_status(state, Admin_ERROR, "Invalid or expired authenticator code.");
        }
    ...
    if (!AUTH_DB_set_totp(state->username, state->totp_secret, state->acting_admin,
                          error, sizeof(error))) {
        admin_set_status(state, Admin_ERROR, error);
        return;
    }
...
static void admin_submit_remove_totp(Type_Auth_Admin_State *state) {
    ...
    if (!state || !AUTH_DB_remove_totp(state->username, state->acting_admin,
                                       error, sizeof(error))) {
        if (state) {
...
static void admin_submit_reset(Type_Auth_Admin_State *state) {
    ...
    if (!AUTH_DB_reset_password(state->username, state->password, state->acting_admin,
                                error, sizeof(error))) {
        admin_set_status(state, Admin_ERROR, error);
        return;
    }
...
static void admin_submit_role(Type_Auth_Admin_State *state) {
    ...
    if (!admin_can_change_selected_role(state)) {
        admin_set_status(state, Admin_ERROR,
                         "The selected account role cannot be changed.");
        return;
    }
    selected = &state->users[state->selected];
    new_role = selected->role == AUTH_ROLE_CO_ADMIN
                   ? AUTH_ROLE_USER
                   : AUTH_ROLE_CO_ADMIN;

    if (!AUTH_DB_set_role(selected->username, new_role, state->acting_admin,
                          error, sizeof(error))) {
        admin_set_status(state, Admin_ERROR,
                         error[0] ? error : "Unable to update the account role.");
        return;
    }

    admin_refresh_users(state);
    admin_set_status(state, new_role == AUTH_ROLE_CO_ADMIN ? Admin_COADMIN : Admin_TEXT,
                     new_role == AUTH_ROLE_CO_ADMIN
                         ? "The selected account is now a co-administrator."
...
static int admin_handle_mouse(Type_Auth_Admin_State *state, int mouse_x, int mouse_y, int width, int height,
    ...
    if (state->view == AUTH_ADMIN_VIEW_USERS) {
        ...
        if (admin_can_modify_selected(state) &&
            admin_point_in_rect(mouse_x, mouse_y, reset_button)) {
            admin_clear_form(state);
        ...
        if (admin_can_delete_selected(state) &&
            admin_point_in_rect(mouse_x, mouse_y, delete_button)) {
            admin_clear_form(state);
            admin_copy(state->username, sizeof(state->username), state->users[state->selected].username);
        ...
        if (admin_can_modify_selected(state) &&
            admin_point_in_rect(mouse_x, mouse_y, set_totp_button)) {
            admin_clear_form(state);
            admin_copy(state->username, sizeof(state->username), state->users[state->selected].username);
            if (!AUTH_TOTP_generate_secret(state->totp_secret)) {
                admin_set_status(state, Admin_ERROR, "Unable to generate a secure 2FA secret.");
...
int AUTH_ADMIN_run(SDL_Window *window, SDL_Renderer *renderer, TTF_Font *font_small,
                   TTF_Font *font_medium, const char *authenticated_admin, int bootstrap_mode) {
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                ...
                int height = 0;
                SDL_GetWindowSize(window, &width, &height);
                bootstrap_created = admin_handle_mouse(&state, event.button.x, event.button.y, width, height,
                                                       &running);
                if (bootstrap_created) {
                    running = 0;
                }
