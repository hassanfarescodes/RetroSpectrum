...
#define SERVER_IDENTITY_PUBLIC_FILE_BYTES \
    (SERVER_IDENTITY_PUBLIC_FILE_DIGEST_OFFSET + SERVER_IDENTITY_SHA512_BYTES)
...
static int server_identity_write_name_file(const char *path, const char *name) {
    ...
    if (!server_identity_write_all(descriptor, (const unsigned char *)line, (size_t)length) ||
...
static int server_identity_sha512_parts(const unsigned char *part1, size_t part1_size,
                                        const unsigned char *part2, size_t part2_size,
                                        const unsigned char *part3, size_t part3_size,
...
static int server_identity_write_key_file(const char *path,
    ...
    if (!server_identity_write_all(descriptor, file_data, SERVER_IDENTITY_KEY_FILE_BYTES) ||
        fsync(descriptor) != 0 || close(descriptor) != 0) {
        descriptor = -1;
        unlink(path);
        goto cleanup;
...
static int server_identity_read_key_file(const char *path,
    ...
    if (memcmp(file_data, SERVER_IDENTITY_KEY_MAGIC, 8) != 0 ||
        file_data[8] != SERVER_IDENTITY_PACKET_VERSION || file_data[9] != 87U ||
        !server_identity_key_file_digest(file_data, expected_digest) ||
        CRYPTO_memcmp(expected_digest, file_data + SERVER_IDENTITY_KEY_DIGEST_OFFSET,
...
static void server_identity_fingerprint_from_public_key(
    const unsigned char public_key[SERVER_IDENTITY_PUBLIC_KEY_BYTES],
...
static int server_identity_public_file_digest(const unsigned char *file_data,
    ...
    return file_data && server_identity_sha512_parts(domain, sizeof(domain) - 1, file_data,
                                                       SERVER_IDENTITY_PUBLIC_FILE_DIGEST_OFFSET,
                                                       NULL, 0, digest);
}
...
static int server_identity_encode_public_file(const char *server_name,
    ...
    memcpy(file_data + SERVER_IDENTITY_PUBLIC_FILE_KEY_OFFSET, public_key,
           SERVER_IDENTITY_PUBLIC_KEY_BYTES);
...
static int server_identity_decode_public_file(const unsigned char file_data[SERVER_IDENTITY_PUBLIC_FILE_BYTES],
    ...
    if (!file_data || !identity ||
        memcmp(file_data, SERVER_IDENTITY_PUBLIC_FILE_MAGIC, 8) != 0 ||
    ...
    if (name_length == 0 || name_length > SERVER_IDENTITY_SERVER_NAME_MAX ||
        ...
        CRYPTO_memcmp(expected_digest,
                      file_data + SERVER_IDENTITY_PUBLIC_FILE_DIGEST_OFFSET,
                      SERVER_IDENTITY_SHA512_BYTES) != 0) {
        server_identity_secure_zero(expected_digest, sizeof(expected_digest));
        return 0;
    }
    ...
    memcpy(identity->public_key, file_data + SERVER_IDENTITY_PUBLIC_FILE_KEY_OFFSET,
           SERVER_IDENTITY_PUBLIC_KEY_BYTES);
...
static int server_identity_read_public_file(const char *path, Type_Server_Public_Identity *identity) {
    ...
    if (!file_data || !server_identity_read_all(descriptor, file_data,
                                                 SERVER_IDENTITY_PUBLIC_FILE_BYTES) ||
...
static int server_identity_write_public_file(const char *path, const char *server_name,
    ...
    umask(previous_mask);
    if (descriptor < 0 ||
        !server_identity_write_all(descriptor, file_data, SERVER_IDENTITY_PUBLIC_FILE_BYTES) ||
        fsync(descriptor) != 0 || close(descriptor) != 0) {
        descriptor = -1;
...
static void server_identity_set_trusted(const Type_Server_Public_Identity *identity) {
    ...
    snprintf(Global_Server_Identity_Trusted_Fingerprint,
             sizeof(Global_Server_Identity_Trusted_Fingerprint), "%s",
             identity->fingerprint_sha512);
    memcpy(Global_Server_Identity_Trusted_Public, identity->public_key,
           SERVER_IDENTITY_PUBLIC_KEY_BYTES);
    ...
    snprintf(Global_Server_Identity_Trusted_Host,
             sizeof(Global_Server_Identity_Trusted_Host), "%s",
             Global_Server_Identity_Trusted_Is_Local ? "127.0.0.1" : "");
    Global_Server_Identity_Last_Verified_At =
        Global_Server_Identity_Trusted_Is_Local ? (int64_t)time(NULL) : 0;
    pthread_mutex_unlock(&Global_Server_Identity_Lock);
}
...
static int server_identity_initialize_public_files(void) {
    ...
    if (!server_identity_named_path(Global_Server_Identity_Public_File_Path,
                                    sizeof(Global_Server_Identity_Public_File_Path),
                                    "server_identity_mldsa87.rspub") ||
        !server_identity_write_public_file(Global_Server_Identity_Public_File_Path,
                                           Global_Server_Identity_Local_Name,
    ...
    if (stat(trusted_path, &st) == 0) {
        if (!server_identity_read_public_file(trusted_path, &identity)) {
            server_identity_set_status("The trusted server public-key file is invalid; refusing to replace it automatically.");
            return 0;
        }
        if (CRYPTO_memcmp(identity.public_key, Global_Server_Identity_Public,
                          SERVER_IDENTITY_PUBLIC_KEY_BYTES) == 0) {
            snprintf(identity.server_name, sizeof(identity.server_name), "%s",
                     Global_Server_Identity_Local_Name);
            server_identity_fingerprint_from_public_key(identity.public_key,
                                                        identity.fingerprint_sha512);
            (void)server_identity_write_public_file(trusted_path, identity.server_name,
                                                    identity.public_key);
        }
...
static int server_identity_build_packet(unsigned char packet[SERVER_IDENTITY_PACKET_BYTES], unsigned char type) {
    ...
    memcpy(packet + SERVER_IDENTITY_PUBLIC_OFFSET, Global_Server_Identity_Public,
           SERVER_IDENTITY_PUBLIC_KEY_BYTES);
    ...
    return server_identity_sign_with_private(Global_Server_Identity_Private, packet,
                                              SERVER_IDENTITY_SIGNED_BYTES,
...
static int server_identity_packet_valid(const unsigned char packet[SERVER_IDENTITY_PACKET_BYTES],
    ...
    if (memcmp(packet, SERVER_IDENTITY_MAGIC, SERVER_IDENTITY_MAGIC_BYTES) != 0 ||
        packet[8] != SERVER_IDENTITY_PACKET_VERSION ||
        (packet[9] != SERVER_IDENTITY_PACKET_TYPE_ANNOUNCE &&
         packet[9] != SERVER_IDENTITY_PACKET_TYPE_QUERY)) {
        return 0;
    }
...
static void server_identity_mark_conflict(const struct sockaddr_in *source,
    ...
    snprintf(message, sizeof(message),
             "Short-label collision observed for %s from %s (SHA-512 %.16s...). Trusted public-key validation is unaffected.",
...
static void server_identity_refresh_local_validation(const unsigned char *packet,
                                                     size_t packet_size) {
    char packet_id[SERVER_IDENTITY_ID_BUFFER];
    unsigned char packet_public[SERVER_IDENTITY_PUBLIC_KEY_BYTES];
    int valid;
    ...
    if (valid) {
        ...
        if (Global_Server_Identity_Trusted_Is_Local &&
            CRYPTO_memcmp(packet_public, Global_Server_Identity_Public,
                          SERVER_IDENTITY_PUBLIC_KEY_BYTES) == 0 &&
            CRYPTO_memcmp(packet_public, Global_Server_Identity_Trusted_Public,
                          SERVER_IDENTITY_PUBLIC_KEY_BYTES) == 0) {
            Global_Server_Identity_Last_Verified_At = (int64_t)time(NULL);
            if (!Global_Server_Identity_Conflict) {
                snprintf(Global_Server_Identity_Status,
                         sizeof(Global_Server_Identity_Status), "%s",
...
static void server_identity_handle_packet(const unsigned char *packet, size_t packet_size,
    ...
    if (source &&
        ...
        if (inet_ntop(AF_INET, &source->sin_addr, address, sizeof(address))) {
            pthread_mutex_lock(&Global_Server_Identity_Lock);
            snprintf(Global_Server_Identity_Trusted_Host,
                     sizeof(Global_Server_Identity_Trusted_Host), "%s", address);
            Global_Server_Identity_Last_Verified_At = (int64_t)time(NULL);
            if (!Global_Server_Identity_Conflict) {
                snprintf(Global_Server_Identity_Status,
                         sizeof(Global_Server_Identity_Status),
                         "%s",
...
int SERVER_IDENTITY_start(void) {
    ...
    server_identity_fingerprint_from_public_key(Global_Server_Identity_Public,
                                                Global_Server_Identity_Fingerprint);
...
static int server_identity_resolve_import_path(const char *path, char *resolved,
...
int SERVER_IDENTITY_validate_target(const char *server_id, char *message, size_t message_size) {
    ...
    if (message && message_size > 0) {
        if (strcmp(server_id, Global_Server_Identity_Id) == 0) {
            ...
            snprintf(message, message_size, "Remote server selected; ML-DSA-87 authenticated hybrid post-quantum TLS is required.");
        }
    }
    return 1;
}
...
int SERVER_IDENTITY_get_trusted_public(unsigned char public_key[SERVER_IDENTITY_PUBLIC_KEY_BYTES]) {
    ...
    pthread_mutex_lock(&Global_Server_Identity_Lock);
    memcpy(public_key, Global_Server_Identity_Trusted_Public,
           SERVER_IDENTITY_PUBLIC_KEY_BYTES);
    pthread_mutex_unlock(&Global_Server_Identity_Lock);
    return 1;
}
...
int SERVER_IDENTITY_get_local_public(unsigned char public_key[SERVER_IDENTITY_PUBLIC_KEY_BYTES]) {
    ...
    pthread_mutex_lock(&Global_Server_Identity_Lock);
    memcpy(public_key, Global_Server_Identity_Public,
           SERVER_IDENTITY_PUBLIC_KEY_BYTES);
    pthread_mutex_unlock(&Global_Server_Identity_Lock);
    return 1;
}
...
int SERVER_IDENTITY_sign_local(const unsigned char *message, size_t message_size,
    ...
    pthread_mutex_lock(&Global_Server_Identity_Lock);
    result = server_identity_sign_with_private(Global_Server_Identity_Private,
