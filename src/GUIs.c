...
void ANALYSIS_draw_line_plot(SDL_Renderer *renderer,
    ...
    */
    ...
    if (bipolar) SDL_RenderDrawLine(renderer, rect.x, mid, rect.x + rect.w, mid);
    else SDL_RenderDrawLine(renderer, rect.x, rect.y + rect.h - 12, rect.x + rect.w, rect.y + rect.h - 12);
    ...
    for (int px = 0; px < rect.w; px++) {
        ...
        if (px > 0) SDL_RenderDrawLine(renderer, prev_x, prev_y, x, y);
                                        ...
                                        int max_px)
{
    ...
    */
    ...
    if (TTF_SizeText(font, dst, &text_w, &text_h) != 0) return;

    if (text_w <= max_px) return;

    const char *ellipsis = "...";
    int ell_w = 0;

    if (TTF_SizeText(font, ellipsis, &ell_w, &text_h) != 0) return;
...
void ANALYSIS_draw_loading_indicator(SDL_Renderer *renderer,
    ...
    */
    ...
    if (!font || !Global_Analysis_Loading) return;

    int box_w = 150;
    int box_h = 28;

    SDL_Rect box = {
        parent_rect.x + parent_rect.w - box_w - 12,
        parent_rect.y + 8,
        box_w,
        box_h
    };
    ...
    draw_text(renderer,
              font,
              frames[Global_Analysis_Load_Frame],
              box.x + 10,
              box.y + 7,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        SDL_Color color = active ?
                          (SDL_Color){0, 255, 90, 230} :
                          (SDL_Color){0, 100, 40, 160};
...
void ANALYSIS_draw_file_list(SDL_Renderer *renderer, TTF_Font *font, SDL_Rect rect){
    ...
    */
    ...
    draw_text(renderer, font,
              "Analysis Workstation | R rescan | Up/Down select | Enter open | Wheel scroll/zoom | G/Esc exit",
              rect.x + 12, rect.y + 12,
    ...
    ANALYSIS_make_ellipsis_text(font,
                                Global_Analysis_Status,
                                short_status,
                                sizeof(short_status),
                                rect.w - 24);

    SDL_Rect status_clip = {
        rect.x + 8,
        rect.y + 38,
        rect.w - 16,
        24
    ...
    draw_text(renderer, font, short_status, rect.x + 12, rect.y + 40,
