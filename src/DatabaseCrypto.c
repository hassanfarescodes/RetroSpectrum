...
static int database_crypto_ensure_directory(const char *path) {
    ...
    return lstat(path, &st) == 0 && S_ISDIR(st.st_mode) && !S_ISLNK(st.st_mode) &&
           st.st_uid == geteuid() && access(path, R_OK | W_OK | X_OK) == 0;
}
...
static int database_crypto_path(char *path, size_t path_size, const char *filename) {
    ...
    if (!path || path_size == 0 || !filename ||
        !database_crypto_directory(directory, sizeof(directory))) {
        return 0;
    }
...
static void database_crypto_cleanup_cached_master(void) {
    ...
    if (Global_Database_Crypto_Master_Loaded) {
        OPENSSL_cleanse(Global_Database_Crypto_Master,
                        sizeof(Global_Database_Crypto_Master));
        (void)munlock(Global_Database_Crypto_Master,
                      sizeof(Global_Database_Crypto_Master));
...
static void database_crypto_cache_master(
    const unsigned char master[DATABASE_CRYPTO_MASTER_BYTES]) {
    pthread_mutex_lock(&Global_Database_Crypto_Key_Lock);
    if (Global_Database_Crypto_Master_Loaded) {
        OPENSSL_cleanse(Global_Database_Crypto_Master,
                        sizeof(Global_Database_Crypto_Master));
    }
    memcpy(Global_Database_Crypto_Master, master,
           sizeof(Global_Database_Crypto_Master));
    Global_Database_Crypto_Master_Loaded = 1;
    (void)mlock(Global_Database_Crypto_Master,
                sizeof(Global_Database_Crypto_Master));
#ifdef MADV_DONTDUMP
    (void)madvise(Global_Database_Crypto_Master,
                  sizeof(Global_Database_Crypto_Master), MADV_DONTDUMP);
#endif
...
static int database_crypto_copy_cached_master(
    ...
    if (available) {
        memcpy(master, Global_Database_Crypto_Master,
               DATABASE_CRYPTO_MASTER_BYTES);
    }
    pthread_mutex_unlock(&Global_Database_Crypto_Key_Lock);
    return available;
}
...
static int database_crypto_read_saved_key_path(char *path, size_t path_size) {
    ...
    if (!path || path_size == 0 ||
        !database_crypto_path(config_path, sizeof(config_path),
                              DATABASE_CRYPTO_KEY_PATH_FILENAME)) {
        return -1;
    }
    ...
    if (fstat(descriptor, &st) != 0 || !S_ISREG(st.st_mode) ||
...
static int database_crypto_resolve_key_path(char *path, size_t path_size,
                                            char *error, size_t error_size) {
    char resolved[PATH_MAX] = "";

    if (!path || path_size == 0) {
        database_crypto_error(error, error_size,
                              "Invalid database key-path destination.");
        return 0;
    }
    ...
    if (Global_Database_Crypto_Key_Path[0] != '\0') {
        snprintf(resolved, sizeof(resolved), "%s",
                 Global_Database_Crypto_Key_Path);
    }
    pthread_mutex_unlock(&Global_Database_Crypto_Key_Lock);

    if (resolved[0] == '\0') {
        int saved_result =
            database_crypto_read_saved_key_path(resolved, sizeof(resolved));
        if (saved_result < 0) {
            database_crypto_error(
                error, error_size,
                "The saved database key-file path is invalid or unsafe.");
            return 0;
        }
        if (saved_result == 0 &&
            !database_crypto_default_key_path(resolved, sizeof(resolved))) {
            database_crypto_error(
                error, error_size,
                "Unable to resolve the database master-key path.");
            return 0;
        }
    }
    if (strlen(resolved) >= path_size) {
        database_crypto_error(error, error_size,
                              "The database master-key path is too long.");
        return 0;
    }

    pthread_mutex_lock(&Global_Database_Crypto_Key_Lock);
    if (Global_Database_Crypto_Key_Path[0] == '\0') {
        snprintf(Global_Database_Crypto_Key_Path,
                 sizeof(Global_Database_Crypto_Key_Path), "%s", resolved);
    }
...
static int database_crypto_load_key_file(
    const char *path, unsigned char master[DATABASE_CRYPTO_MASTER_BYTES],
    char *error, size_t error_size) {
    int descriptor;

    if (!path || path[0] == '\0' || !master) {
        database_crypto_error(error, error_size,
                              "No database key file was selected.");
        return 0;
    }
    ...
    if (descriptor < 0) {
        ...
        snprintf(message, sizeof(message),
                 "Unable to open database key file %s: %s", path,
                 strerror(errno));
        database_crypto_error(error, error_size, message);
        return 0;
    }
    if (!database_crypto_validate_key_file(descriptor) ||
        !database_crypto_read_all(descriptor, master,
                                  DATABASE_CRYPTO_MASTER_BYTES)) {
        close(descriptor);
        database_crypto_error(
            error, error_size,
            "The database key file must be an owner-only regular 32-byte file.");
        return 0;
    }
    close(descriptor);
    return 1;
...
static int database_crypto_any_database_exists(void) {
    ...
    if (database_crypto_path(auth_path, sizeof(auth_path),
                             DATABASE_CRYPTO_AUTH_FILENAME) &&
        lstat(auth_path, &st) == 0) {
        return 1;
    }
    if (database_crypto_path(data_path, sizeof(data_path),
                             DATABASE_CRYPTO_DATA_FILENAME) &&
...
static int database_crypto_create_default_master(
    const char *path, unsigned char master[DATABASE_CRYPTO_MASTER_BYTES],
    char *error, size_t error_size) {
    int descriptor = -1;
    mode_t old_mask;
    ...
    if (RAND_priv_bytes(master, DATABASE_CRYPTO_MASTER_BYTES) != 1) {
        database_crypto_error(error, error_size,
                              "Unable to generate a database master key.");
        return 0;
    }
    old_mask = umask(0077);
    ...
    if (descriptor < 0) {
        database_crypto_error(
            error, error_size,
            "Unable to securely create the local database master-key file.");
        goto cleanup;
    }
    if (!database_crypto_write_all(descriptor, master,
                                   DATABASE_CRYPTO_MASTER_BYTES) ||
        fsync(descriptor) != 0 || fchmod(descriptor, 0600) != 0) {
        database_crypto_error(error, error_size,
                              "Unable to persist the database master key.");
        close(descriptor);
        descriptor = -1;
        unlink(path);
        goto cleanup;
...
static int database_crypto_load_or_create_master(
    unsigned char master[DATABASE_CRYPTO_MASTER_BYTES], char *error,
    size_t error_size) {
    char path[PATH_MAX];
    ...
    if (!master) {
        database_crypto_error(error, error_size,
                              "Invalid database master-key buffer.");
        return 0;
    }
    if (database_crypto_copy_cached_master(master)) {
        return 1;
    }
    if (!database_crypto_resolve_key_path(path, sizeof(path), error,
                                          error_size) ||
        !database_crypto_default_key_path(default_path,
                                          sizeof(default_path))) {
        return 0;
    }
    {
        ...
        if (!database_crypto_load_key_file(path, master, error, error_size)) {
            if (!key_missing || strcmp(path, default_path) != 0 ||
                database_crypto_any_database_exists()) {
                OPENSSL_cleanse(master, DATABASE_CRYPTO_MASTER_BYTES);
                return 0;
            }
            if (!database_crypto_create_default_master(path, master, error,
                                                       error_size)) {
                return 0;
            }
        }
...
static int database_crypto_derive_from_master(
    const unsigned char master[DATABASE_CRYPTO_MASTER_BYTES],
    ...
    if (!master || !domain || !key ||
        !HMAC(EVP_sha512(), master, DATABASE_CRYPTO_MASTER_BYTES,
              (const unsigned char *)domain, strlen(domain), digest,
              &digest_size) ||
        digest_size < DATABASE_CRYPTO_DERIVED_BYTES) {
        database_crypto_error(error, error_size,
                              "Unable to derive the database encryption key.");
        goto cleanup;
    }
...
static int database_crypto_derive_key(
    const char *domain, unsigned char key[DATABASE_CRYPTO_DERIVED_BYTES],
    char *error, size_t error_size) {
    unsigned char master[DATABASE_CRYPTO_MASTER_BYTES];
    int success;
    ...
    if (!domain || !key ||
        !database_crypto_load_or_create_master(master, error, error_size)) {
        OPENSSL_cleanse(master, sizeof(master));
        return 0;
    }
    success = database_crypto_derive_from_master(master, domain, key, error,
                                                 error_size);
    OPENSSL_cleanse(master, sizeof(master));
    return success;
}
...
static int database_crypto_exec(sqlite3 *database, const char *sql, char *error, size_t error_size) {
    ...
    if (result != SQLITE_OK) {
        database_crypto_error(error, error_size,
                              sqlite_error ? sqlite_error : sqlite3_errmsg(database));
...
static int database_crypto_validate_database_file(const char *path, char *error,
                                                   size_t error_size) {
    struct stat st;
...
static int database_crypto_apply_key(sqlite3 *database,
                                     const unsigned char key[DATABASE_CRYPTO_DERIVED_BYTES],
                                     char *error, size_t error_size) {
    ...
    if (!database || !key || !database_crypto_has_sqlcipher(database)) {
        database_crypto_error(error, error_size,
                              "RetroSpectrum requires SQLCipher 4.6.1 or newer; plaintext SQLite and older SQLCipher builds are refused.");
        return 0;
    }
...
static int database_crypto_key_opens_database(
    ...
    if (descriptor < 0) {
        ...
        database_crypto_error(error, error_size,
                              "Unable to inspect an existing database file.");
        return 0;
    }
    ...
    if (result == (int)sizeof(header) &&
        memcmp(header, "SQLite format 3\000", sizeof(header)) == 0) {
        return 1;
    }

    memset(key, 0, sizeof(key));
    if (!database_crypto_derive_from_master(master, domain, key, error,
                                            error_size)) {
        goto cleanup;
    }
    result = sqlite3_open_v2(path, &database,
                             ...
                             NULL);
    if (result != SQLITE_OK ||
        !database_crypto_apply_key(database, key, error, error_size) ||
        sqlite3_prepare_v2(database, "SELECT count(*) FROM sqlite_master;",
...
static int database_crypto_write_saved_key_path(const char *path, char *error,
                                                size_t error_size) {
    char config_path[PATH_MAX];
    ...
    if (!path ||
        !database_crypto_path(config_path, sizeof(config_path),
                              DATABASE_CRYPTO_KEY_PATH_FILENAME) ||
        snprintf(temporary, sizeof(temporary), "%s.tmp-%ld", config_path,
                 (long)getpid()) >= (int)sizeof(temporary)) {
        database_crypto_error(error, error_size,
                              "Unable to save the database key-file path.");
        return 0;
    }
    length = strlen(path);
    old_mask = umask(0077);
    ...
    umask(old_mask);
    if (descriptor < 0 ||
        !database_crypto_write_all(descriptor, (const unsigned char *)path,
                                   length) ||
        !database_crypto_write_all(descriptor,
                                   (const unsigned char *)"\n", 1) ||
    ...
    if (rename(temporary, config_path) != 0 || chmod(config_path, 0600) != 0) {
        unlink(temporary);
        database_crypto_error(error, error_size,
                              "Unable to activate the database key-file path.");
        return 0;
    }
    success = 1;
    return success;
...
static int database_crypto_parent_directory_writable(const char *path, char *error,
                                                       size_t error_size) {
    char directory[PATH_MAX];
    char *separator;
    struct stat st;
    ...
    if (lstat(directory, &st) != 0) {
        if (error && error_size > 0) {
            snprintf(error, error_size, "Unable to inspect database directory %s: %s",
    ...
    if (!S_ISDIR(st.st_mode) || S_ISLNK(st.st_mode)) {
        database_crypto_error(error, error_size,
                              "The database parent path is not a safe directory.");
        return 0;
    }
    if (st.st_uid != geteuid()) {
        if (error && error_size > 0) {
            snprintf(error, error_size,
                     "Database directory %s is not owned by the current user.", directory);
        }
        return 0;
    }
    if ((st.st_mode & 077) != 0 && chmod(directory, 0700) != 0) {
        if (error && error_size > 0) {
            snprintf(error, error_size, "Unable to restrict database directory %s: %s",
                     directory, strerror(errno));
        }
        return 0;
    }
    if (access(directory, R_OK | W_OK | X_OK) != 0) {
        if (error && error_size > 0) {
            snprintf(error, error_size, "Database directory %s is not writable: %s",
                     directory, strerror(errno));
        }
        return 0;
    }
...
static int database_crypto_create_secure_empty_file(const char *path, char *error,
                                                     size_t error_size) {
    int descriptor;
    ...
    if (descriptor < 0) {
        if (error && error_size > 0) {
            snprintf(error, error_size, "Unable to create temporary SQLCipher database %s: %s",
                     path, strerror(errno));
        }
        return 0;
    }
    ...
    if (fchmod(descriptor, 0600) != 0 || fsync(descriptor) != 0) {
        ...
        unlink(path);
        if (error && error_size > 0) {
            snprintf(error, error_size, "Unable to initialize temporary SQLCipher database %s: %s",
    ...
    if (close(descriptor) != 0) {
        ...
        unlink(path);
        if (error && error_size > 0) {
            snprintf(error, error_size, "Unable to close temporary SQLCipher database %s: %s",
                     path, strerror(saved_errno));
        }
        return 0;
    }
...
static int database_crypto_export_plaintext(
    sqlite3 *source,
    ...
    attach_sql = sqlite3_mprintf(
        "ATTACH DATABASE %Q AS encrypted KEY \"x'%s'\";",
        destination_path,
        key_hex);
    if (!attach_sql) {
...
static int database_crypto_verify_encrypted_copy(
    const char *path,
    const unsigned char key[DATABASE_CRYPTO_DERIVED_BYTES],
    char *error,
    size_t error_size) {
...
static int database_crypto_migrate_plaintext(const char *path,
                                             const unsigned char key[DATABASE_CRYPTO_DERIVED_BYTES],
                                             char *error, size_t error_size) {
    sqlite3 *source = NULL;
    char temporary[PATH_MAX];
    ...
    if (snprintf(temporary, sizeof(temporary), "%s.sqlcipher-new-%ld", path, (long)getpid()) >=
        ...
        snprintf(backup, sizeof(backup), "%s.plaintext-old-%ld", path, (long)getpid()) >=
            (int)sizeof(backup)) {
    ...
    if ((unlink(temporary) != 0 && errno != ENOENT) ||
        (unlink(backup) != 0 && errno != ENOENT)) {
        database_crypto_error(error, error_size,
                              "Unable to remove a stale database migration file.");
        return 0;
    }
    ...
    if (chmod(temporary, 0600) != 0 ||
        !database_crypto_fsync_file(temporary) ||
...
static int database_crypto_open(sqlite3 **database, char *path, size_t path_size,
    ...
    if (!database || !path || !filename || !domain ||
        !database_crypto_path(path, path_size, filename) ||
        !database_crypto_derive_key(domain, key, error, error_size)) {
        return 0;
    }

    if (!database_crypto_validate_database_file(path, error, error_size) ||
        (database_crypto_file_is_plaintext(path) &&
         !database_crypto_migrate_plaintext(path, key, error, error_size))) {
        goto cleanup;
    }
    ...
    if (chmod(path, 0600) != 0 ||
        !database_crypto_exec(*database, "PRAGMA foreign_keys = ON;", error, error_size) ||
...
int DATABASE_CRYPTO_open_auth(sqlite3 **database, char *path, size_t path_size,
                              char *error, size_t error_size) {
    return database_crypto_open(database, path, path_size, DATABASE_CRYPTO_AUTH_FILENAME,
                                "RetroSpectrum SQLCipher auth database key v1",
                                error, error_size);
}

int DATABASE_CRYPTO_open_data(sqlite3 **database, char *path, size_t path_size,
                              char *error, size_t error_size) {
    return database_crypto_open(database, path, path_size, DATABASE_CRYPTO_DATA_FILENAME,
                                "RetroSpectrum SQLCipher operational database key v1",
                                error, error_size);
}

int DATABASE_CRYPTO_set_key_path(const char *path, char *error,
                                 size_t error_size) {
    ...
    if (!path || path[0] != '/' || !realpath(path, canonical)) {
        database_crypto_error(
            error, error_size,
            "Enter an absolute path to an existing 32-byte database key file.");
        goto cleanup;
    }
    if (!database_crypto_load_key_file(canonical, master, error, error_size) ||
        !database_crypto_path(auth_path, sizeof(auth_path),
                              DATABASE_CRYPTO_AUTH_FILENAME) ||
        !database_crypto_path(data_path, sizeof(data_path),
                              DATABASE_CRYPTO_DATA_FILENAME) ||
        !database_crypto_key_opens_database(
            auth_path, "RetroSpectrum SQLCipher auth database key v1", master,
            error, error_size) ||
        !database_crypto_key_opens_database(
            data_path,
            "RetroSpectrum SQLCipher operational database key v1", master,
            error, error_size) ||
    ...
    pthread_mutex_lock(&Global_Database_Crypto_Key_Lock);
    snprintf(Global_Database_Crypto_Key_Path,
             sizeof(Global_Database_Crypto_Key_Path), "%s", canonical);
    pthread_mutex_unlock(&Global_Database_Crypto_Key_Lock);
    database_crypto_cache_master(master);
    success = 1;
