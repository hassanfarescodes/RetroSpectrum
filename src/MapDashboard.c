 ...
 * Author:          Hassan Fares
 *
 * Description:     Dashboard and world-map/case rendering logic for RetroSpectrum.
 *                  This file was split out of RetroSpectrum.c so RetroSpectrum.c
...
void dashboard_draw_text_centered(SDL_Renderer *renderer,
                                         TTF_Font *font,
                                         const char *text,
                                         SDL_Rect rect,
                                         SDL_Color color){
...
void dashboard_draw_station_card(SDL_Renderer *renderer,
                                        TTF_Font *font_small,
                                        TTF_Font *font_medium,
                                        SDL_Rect rect,
                                        const char *title,
...
void dashboard_draw_case_points(Type_Dashboard_State *dashboard,
...
void dashboard_draw_hover_country_cases(Type_Dashboard_State *dashboard,
                                               SDL_Renderer *renderer,
                                               TTF_Font *font,
                                               SDL_Rect sidebar){
...
void dashboard_draw_case_search(Type_Dashboard_State *dashboard,
                                       SDL_Renderer *renderer,
                                       TTF_Font *font,
                                       SDL_Rect area){
...
static void dashboard_set_case_description_cursor_from_mouse(Type_Dashboard_State *dashboard, SDL_Rect rect, int mouse_x, int mouse_y);
...
void dashboard_draw_case_sidebar(Type_Dashboard_State *dashboard,
                                        SDL_Renderer *renderer,
                                        TTF_Font *font,
                                        SDL_Rect sidebar){
    ...
    SDL_Rect desc_text_rect = {dashboard->case_desc_rect.x + 9,
    ...
    if (dashboard->case_desc_editing) {
        ...
        if (dashboard_case_desc_selection_range(&sel_a, &sel_b)) {
            ...
            for (int line = 0; line < line_count; line++) {
                ...
                if (a < b) {
                    int x0 = desc_text_rect.x + dashboard_text_range_width(font, dashboard->case_desc_edit, (size_t)line_a, (size_t)a);
                    int x1 = desc_text_rect.x + dashboard_text_range_width(font, dashboard->case_desc_edit, (size_t)line_a, (size_t)b);
    ...
    if (dashboard->case_desc_editing && ((SDL_GetTicks64() / 520ULL) % 2ULL) == 0ULL) {
        ...
        for (int line = 0; line < line_count; line++) {
            if (cursor >= starts[line] && cursor <= ends[line]) {
                int cx = desc_text_rect.x + dashboard_text_range_width(font,
                ...
                if (cx < desc_text_rect.x) cx = desc_text_rect.x;
                if (cx > desc_text_rect.x + desc_text_rect.w) cx = desc_text_rect.x + desc_text_rect.w;
...
static void dashboard_clamp_case_desc_cursor(Type_Dashboard_State *dashboard){
    ...
    if (Global_Dashboard_Case_Desc_Cursor < 0) Global_Dashboard_Case_Desc_Cursor = 0;
    if (Global_Dashboard_Case_Desc_Cursor > len) Global_Dashboard_Case_Desc_Cursor = len;
}
...
static int dashboard_case_desc_delete_selection(Type_Dashboard_State *dashboard){
    ...
    if (!dashboard || !dashboard_case_desc_selection_range(&a, &b)) return 0;
    ...
    memmove(dashboard->case_desc_edit + a,
            dashboard->case_desc_edit + b,
            (size_t)(len - b) + 1U);
...
static void dashboard_auto_wrap_text_field_px(char *text,
                                              size_t text_size,
                                              int *cursor,
                                              TTF_Font *font,
                                              int max_px){
    ...
    if (!text || text_size == 0 || max_px < 16) return;
    ...
    while (line_start < len) {
        ...
        while (line_end > segment_start &&
               dashboard_text_range_width(font, text, segment_start, line_end) > max_px) {
            ...
            if (found_space) {
                ...
                if (cursor && *cursor >= (int)break_pos) (*cursor)++;
                segment_start = break_pos + 1;
            }
        }
...
static void dashboard_backspace_case_description(Type_Dashboard_State *dashboard){
    ...
    if (!dashboard) return;
    if (dashboard_case_desc_delete_selection(dashboard)) return;
    ...
    memmove(dashboard->case_desc_edit + cursor - 1,
            dashboard->case_desc_edit + cursor,
...
static void dashboard_delete_case_description(Type_Dashboard_State *dashboard){
    ...
    memmove(dashboard->case_desc_edit + cursor,
            dashboard->case_desc_edit + cursor + 1,
            (size_t)(len - cursor));
}

static void dashboard_set_case_description_cursor_from_mouse(Type_Dashboard_State *dashboard,
                                                             SDL_Rect rect,
                                                             int mouse_x,
                                                             int mouse_y){
    int starts[128];
    ...
    for (int i = 0; i <= line_len; i++) {
        int w0 = dashboard_text_range_width(Global_Dashboard_Case_Desc_Font,
                                            dashboard->case_desc_edit,
                                            (size_t)starts[line],
                                            (size_t)(starts[line] + i));
        int w1 = w0;
        if (i < line_len) {
            w1 = dashboard_text_range_width(Global_Dashboard_Case_Desc_Font,
                                            dashboard->case_desc_edit,
                                            (size_t)starts[line],
    ...
    if (dashboard->case_desc_editing) {
        if (event->type == SDL_MOUSEBUTTONDOWN && event->button.button == SDL_BUTTON_LEFT) {
            if (dashboard_point_in_rect(event->button.x, event->button.y, dashboard->case_desc_rect)) {
                dashboard_set_case_description_cursor_from_mouse(dashboard,
                                                                 dashboard->case_desc_rect,
                                                                 event->button.x,
                                                                 event->button.y);
        ...
        if (event->type == SDL_KEYDOWN) {
            ...
            if (key == SDLK_LEFT) {
                dashboard_clamp_case_desc_cursor(dashboard);
                if (Global_Dashboard_Case_Desc_Cursor > 0) Global_Dashboard_Case_Desc_Cursor--;
                dashboard_case_desc_clear_selection();
                return 1;
            }
            if (key == SDLK_RIGHT) {
                dashboard_clamp_case_desc_cursor(dashboard);
                if (Global_Dashboard_Case_Desc_Cursor < len) Global_Dashboard_Case_Desc_Cursor++;
                dashboard_case_desc_clear_selection();
                return 1;
            }
            if (key == SDLK_HOME) {
